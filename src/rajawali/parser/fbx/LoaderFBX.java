...
public class LoaderFBX extends AMeshLoader {
	...
	public LoaderFBX(RajawaliRenderer renderer, AssetManager assets, String fileOnSdcard) {
	...
	private ALight buildLight(Model l) {
		...
		case ALight.DIRECTIONAL_LIGHT: // Area
			DirectionalLight lD = new DirectionalLight(0, -1, 0); // TODO calculate direction based on position and
																	// rotation
		...
		for (int i = 0; i < numTex; ++i) {
			...
			for (int j = 0; j < numCon; ++j) {
				...
				if (conn.object2.equals(name) && conn.object1.equals(tex.textureName))
				{
					...
					case ASSETS:
						try {
							String filePath = getParentFolder() + File.separatorChar + getOnlyFileName(textureName);
					...
					case RAW:
						int identifier = getResources().getIdentifier(
								getFileNameWithoutExtension(textureName).toLowerCase(Locale.getDefault()), "drawable",
		...
		if (material != null) {
			...
			if (material.shadingModel.equals("phong"))
			{
				...
				if (material.properties.specularColor != null)
				{
					...
					method.setSpecularColor(Color.rgb((int) (color.x * 255.f), (int) (color.y * 255.f),
							(int) (color.z * 255.f)));
				}
		...
		if (line.contains("{")) {
			...
			if (line.contains(":")) {
				if (line.contains(OBJECT_TYPE)) {
					...
					Object te = last
							.getClass()
							.getDeclaredMethod("addTexture", String.class, String.class)
			...
			try {
				...
				if (line.contains(PROPERTY)) {
					...
					if (type.equals(TYPE_VECTOR3D) || type.equals(TYPE_COLOR) || type.equals(TYPE_COLOR_RGB)
							|| type.equals(TYPE_LCL_ROTATION)
							|| type.equals(TYPE_LCL_SCALING) || type.equals(TYPE_LCL_TRANSLATION)
							|| type.equals(TYPE_VECTOR)) {
					...
					last.getClass()
							.getDeclaredMethod("addConnection", String.class, String.class, String.class)
